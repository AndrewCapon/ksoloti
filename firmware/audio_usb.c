...
#else
      int u; for (u=0; u< uBufferAdjust; u++)
      {
...
void aduInitiateTransmitI(USBDriver *usbp)
{
  ...
  if(aduState.state == asInit || aduState.state == asFillingUnderflow)
  {
    ...
    if((aduState.txRingBufferUsedSize == TX_RING_BUFFER_NORMAL_SIZE) && (aduState.rxRingBufferUsedSize >= uRxSamplesRequired))
    {
      ...
      // if(aduState.rxRingBufferUsedSize != 96 || aduState.txRingBufferUsedSize != 192)
      // {
      //   Analyse(GPIOA, 9, 1);
      //   Analyse(GPIOA, 9, 0);
      // }
  ...
  // int16_t *p = pTxLocation;
  // for(int i = 0; i < USE_TRANSFER_SIZE_BYTES/2; i++)
  // {
  //   *p++ = 0x0;
  //   *p++ = 0xffff;
  // }
  //memset(pTxLocation, 0xff, USE_TRANSFER_SIZE_BYTES);
  usbStartTransmitI(usbp, 3, (uint8_t *)pTxLocation, USE_TRANSFER_SIZE_BYTES); // only sending 192 per frame, why????
